        ...
        match self.beneficiary_or_payer:
            ...
            case _:
                if self.iban == "0000000000" and (
                    self.description == "Entgeldabrechnung siehe Anlang" or "Rechnung Berliner Sparkasse Entgelt" in self.description
                ):
...
class AnalyzedBeverageCrate:
    ...
    def __str__(self) -> str:
        return f"{self.beverage.name}: {self.num_ordered}× ordered, {self.num_sold}× sold: {self.total_profit:.2f}€ profit"
