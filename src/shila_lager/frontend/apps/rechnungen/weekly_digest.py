...
from shila_lager.frontend.apps.rechnungen.models import ShilaInventoryCount, ShilaAccountBooking, ShilaBookingCategory, AnalyzedBeverageCrate
...
def output_beverage_consumption_and_expected_profit(analyzed_beverage_crates: dict[BeverageID, AnalyzedBeverageCrate]) -> None:
    ...
    print()
    for crate in sorted(analyzed_beverage_crates.values(), key=lambda it: it.total_profit, reverse=True):
        if all(isclose(it, 0) for it in [crate.total_profit, crate.num_sold, crate.num_returned]):
            continue
        print(f"{bright_color}{(crate.beverage.name + ':').ljust(45, ' ')}{reset_color}{crate.num_sold:.2f} verkauft, \t{crate.num_ordered:.2f} gekauft, \t{crate.num_returned:.2f} zurück, \t{crate.total_profit:.2f}€\t({crate.total_profit_without_deposits:.2f}€) profit")
    pass
    # Expected profit
...
def weekly_digest(start: datetime | None = None, end: datetime | None = None) -> None:
    ...
    averages = np.average(np.array(all_profits, dtype=object), axis=0)
    ...
    print()
    print(f"Standardabweichung: \t{np.std(all_profits[1], axis=0):.2f}")
