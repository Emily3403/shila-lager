...
def import_lager_file(file: Path, inventory_counts: set[ShilaInventoryCount], beverages: dict[str, BeverageCrate]) -> ShilaInventoryCount | None:
    ...
    with open(file) as f:
        ...
        extra_expenses: dict[str, str | int | float] = data.get("Extra Ausgaben") or {}

        assert isinstance(lager_data, dict)
        assert isinstance(money_data, dict)
        assert isinstance(extra_expenses, dict)

        inventory_count = ShilaInventoryCount(date=UTC.localize(parse(file.stem)), money=sum(parse_numeric(money) for money in money_data.values()), extra_expenses=extra_expenses)
